...
/** Always resolve data/ next to the project (lib/../data), not process.cwd(). */
function defaultDataDir() {
  if (process.env.BP_DATA_DIR) return process.env.BP_DATA_DIR;
  // lib/store.js → project root / data
...
class MemoryStore {
  ...
  async purgeFriendFromAllAccounts(friendCode) {
    ...
    for (const acc of this.accounts.values()) {
      ...
      if (acc.friends.length !== before) {
        n++;
        try { await this.saveAccount(acc); } catch (_) {}
      }
    }
    return n;
  }
...
class FileStore {
  ...
  async init() {
    ...
    // Rebuild login index from existing account files (survives client/browser wipes)
    try { this._rebuildAccountIndex(); } catch (_) {}
    return this;
  }
  _rebuildAccountIndex() {
    ...
    for (const f of files) {
      ...
      try {
        ...
        const login = String(acc.login || '').toLowerCase();
        const code = String(acc.friendCode || '').toUpperCase();
        if (login) idx[login] = acc.id;
        if (code) idx['code:' + code] = acc.id;
      } catch (_) {}
    }
    try {
      fs.writeFileSync(path.join(this.accountsDir, '_index.json'), JSON.stringify(idx));
    } catch (_) {}
  }
  _roomPath(id) {
  ...
  _guestProgressPath(code) {
    const c = String(code || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
    return path.join(this.profilesDir, c + '.guest.json');
  }
  async saveGuestProgress(code, data) {
    if (!code) return;
    try {
      fs.mkdirSync(this.profilesDir, { recursive: true });
      fs.writeFileSync(this._guestProgressPath(code), JSON.stringify({
        data: data || {},
  ...
  async loadGuestProgress(code) {
    if (!code) return null;
    try {
      const e = JSON.parse(fs.readFileSync(this._guestProgressPath(code), 'utf8'));
      if (e.exp && Date.now() > e.exp) {
        try { fs.unlinkSync(this._guestProgressPath(code)); } catch (_) {}
        return null;
      }
      return e.data || null;
    } catch (_) { return null; }
  ...
  async saveAccount(acc) {
    if (!acc || !acc.id) return;
    const payload = Object.assign({}, acc, {
      _login: String(acc.login || '').toLowerCase(),
      _code: String(acc.friendCode || '').toUpperCase()
    });
    const dest = this._accountPath(acc.id);
    const tmp = dest + '.tmp';
    // Atomic write so accounts survive crashes / restarts
    fs.writeFileSync(tmp, JSON.stringify(payload, null, 0));
    fs.renameSync(tmp, dest);
    // Maintain login index for reliable lookup after browser data clear
    try {
      const idxPath = path.join(this.accountsDir, '_index.json');
      let idx = {};
      try { idx = JSON.parse(fs.readFileSync(idxPath, 'utf8')); } catch (_) { idx = {}; }
      if (payload._login) idx[payload._login] = acc.id;
      if (payload._code) idx['code:' + payload._code] = acc.id;
      const idxTmp = idxPath + '.tmp';
      fs.writeFileSync(idxTmp, JSON.stringify(idx));
  ...
  async loadAccountByLogin(login) {
    ...
    if (!q) return null;
    // Fast path via index (survives independent of browser / client)
    try {
      const idx = JSON.parse(fs.readFileSync(path.join(this.accountsDir, '_index.json'), 'utf8'));
      if (idx && idx[q]) {
        const acc = await this.loadAccountById(idx[q]);
        if (acc) return acc;
      }
    } catch (_) {}
    let files = [];
  ...
  async loadAccountByCode(code) {
    ...
    try {
      const idx = JSON.parse(fs.readFileSync(path.join(this.accountsDir, '_index.json'), 'utf8'));
      if (idx && idx['code:' + q]) {
        const acc = await this.loadAccountById(idx['code:' + q]);
        if (acc) return acc;
      }
    } catch (_) {}
    let files = [];
  ...
  async deleteAccount(acc) {
    ...
    try {
      ...
      const login = String(acc.login || '').toLowerCase();
      const code = String(acc.friendCode || '').toUpperCase();
      if (login && idx[login] === acc.id) delete idx[login];
      if (code && idx['code:' + code] === acc.id) delete idx['code:' + code];
  ...
  /** Remove friendCode from every account's friends list (scan accountsDir). */
  async purgeFriendFromAllAccounts(friendCode) {
    const code = String(friendCode || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
    if (!code) return 0;
    let files = [];
    try { files = fs.readdirSync(this.accountsDir); } catch (_) { return 0; }
    let n = 0;
    for (const f of files) {
      if (!f.endsWith('.json') || f === '_index.json') continue;
      try {
        const p = path.join(this.accountsDir, f);
        const acc = JSON.parse(fs.readFileSync(p, 'utf8'));
        if (!acc || !Array.isArray(acc.friends) || !acc.friends.length) continue;
        const before = acc.friends.length;
        acc.friends = acc.friends.filter((fr) => {
          const c = String((fr && fr.code) || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
          return c !== code;
        });
        if (acc.friends.length !== before) {
          n++;
...
class RedisStore {
  ...
  async saveGuestProgress(code, data) {
    if (!code) return;
    const key = PREFIX + 'guestprog:' + String(code).toUpperCase();
    try {
      await this.client.set(key, JSON.stringify(data || {}), 30 * 24 * 3600);
    } catch (_) {}
  }
  async loadGuestProgress(code) {
    if (!code) return null;
  ...
  async deleteGuestProgress(code) {
    if (!code) return;
    const key = PREFIX + 'guestprog:' + String(code).toUpperCase();
    try { await this.client.del(key); } catch (_) {}
  }
  ...
  /**
   * Best-effort: Redis store has no full account index.
   * Purge is a no-op here unless a known friends-of set exists.
   * Callers should still notify online peers via social friend_remove.
   */
  async purgeFriendFromAllAccounts(friendCode) {
    return 0;
  }
...
async function createStore() {
  ...
  if (mode === 'redis' || (!mode && redisUrl)) {
    ...
    try {
      ...
      const fsStore = new FileStore(defaultDataDir());
      await fsStore.init();
  ...
  if (mode === 'file' || !mode) {
    const s = new FileStore(defaultDataDir());
    await s.init();
