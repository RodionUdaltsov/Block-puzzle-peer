...
function createAccounts(store) {
  ...
  /**
   * Permanently delete account. Requires valid session; password confirmation recommended.
   * @param {string} token
  ...
  async function deleteAccount(token, opts) {
    ...
    // Optional password confirmation (if provided)
    if (typeof opts.password === 'string' && opts.password.length > 0) {
      if (!verifyPassword(opts.password, acc.passSalt, acc.passHash)) {
    ...
    if (token) await store.deleteSession(token);
    return { ok: true, deleted: true, id: acc.id };
  }
